...
class Stacking_fun():
    ...
    def Stacking_run_Predciting(self, value_list):
        ...
        # Get the first 80% of data
        value_list_80 = value_list[:split_index_80]
        # Get the first 60% of data
        value_list_60 = value_list[:split_index_60]
        # Get the last 20% of data (i.e., 80% to 100%)
        value_list_20 = value_list[split_index_80:]
        # Get data starting from self.window_size
        if(is_decimal_60):
            value_list_60_from_window = value_list_60[self.window_size+1:]
        else:
            value_list_60_from_window = value_list_60[self.window_size:]

        # To get the last 20% of value_list_80, we need to calculate the corresponding index in value_list_80
        split_index_80_in_80 = math.ceil(len(value_list_80) * 0.8)
        # Get the last 20% of value_list_80 (i.e., from 64% to 80% of the original list)
        ...
        # Truncate the predictions from xgboost and svr, and value_list_80_last_20 to ensure consistent length
